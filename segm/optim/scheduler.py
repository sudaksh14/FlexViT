...
class PolynomialLR(_LRScheduler):
    def __init__(self, optimizer, iter_warmup, iter_max, power=0.9, min_lr=1e-6):
        self.iter_warmup = int(iter_warmup)
        self.iter_max = int(iter_max)
        self.power = power
        self.min_lr = min_lr
    ...
    def get_lr(self):
        iter_cur = min(self.last_epoch, self.iter_max)

        if iter_cur < self.iter_warmup:
            coef = iter_cur / self.iter_warmup
        else:
            decay = max(0.0, 1 - iter_cur / self.iter_max)
            coef = decay ** self.power

        return [
            float((base_lr - self.min_lr) * coef + self.min_lr)

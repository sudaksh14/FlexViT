...
from segm.engine_shard import train_one_epoch, evaluate
import wandb
...
@click.option("--normalization", default='deit', type=str)
@click.option("--eval-freq", default=None, type=int)
@click.option("--amp/--no-amp", default=False, is_flag=True)
@click.option("--resume/--no-resume", default=False, is_flag=True)
@click.option("--wb/--no-wb", default=False, is_flag=True)
def main(
    log_dir,
    dataset,
    im_size,
    ...
    if ptu.dist_rank == 0 and wb:
        wandb.init(
            project="segmenter-shard-ade20k",
            name=log_dir.name,
            config=variant,
            dir=str(log_dir),
        )

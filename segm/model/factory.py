...
from segm.model.shard_encoder import *
...
def create_segmenter_shard(model_cfg):
    model_cfg = model_cfg.copy()
    decoder_cfg = model_cfg.pop("decoder")
    decoder_cfg["n_cls"] = model_cfg["n_cls"]

    encoder = load_shard(model_cfg)
    decoders = []
...
def load_shard(model_cfg):
    model = ViTConfig_v3(model_cfg).make_model()
    # model = ViTConfig_v3(**model_cfg).make_model()
    if model_cfg["pretrained_model"] is not None:
        ckpt = torch.load(model_cfg["pretrained_model"], map_location=ptu.device)
        sdict = ckpt["state_dict"] if "state_dict" in ckpt else ckpt
        sdict = {k: v for k, v in sdict.items()
                if "pos_embedding" not in k}
        
        model.load_state_dict(sdict, strict=False)
        print(f"Loaded pretrained model from {model_cfg['pretrained_model']}")
    return model
